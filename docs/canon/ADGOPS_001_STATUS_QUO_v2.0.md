...
### 4.1 Licitaciones shell/footer parity remains unresolved
This is the main currently visible product blocker from the recent lanes.

The previous narrow CSS fix was not enough.
The issue must be re-audited from real files instead of assumed from a one-property theory.
...
### 5.2 Version truth is mixed across surfaces/files
Recent work advanced some areas more than others.
That does not invalidate the repo.
It means the repo must be described honestly as mixed in local progress.
...
- build the core wave-2 documentation frame first
...
Only after that should the project move into per-surface docs.
